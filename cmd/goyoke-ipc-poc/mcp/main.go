...
//
// Flow:
//  1. Read socket path from GOYOKE_SOCKET env var
...
var sampleRequests = []IPCRequest{
	{
		Type:    "modal_request",
		ID:      "req-1",
		Message: "Allow Write to /home/user/.config/goyoke.yaml?",
		Options: []string{"Allow", "Allow Always", "Deny"},
	},
	{
		Type:    "modal_request",
		ID:      "req-2",
		Message: "Allow Bash: rm -rf /tmp/goyoke-cache?",
...
func main() {
	sockPath := os.Getenv("GOYOKE_SOCKET")
	if sockPath == "" {
		fmt.Fprintln(os.Stderr, "[mcp] GOYOKE_SOCKET is not set")
		fmt.Fprintln(os.Stderr, "      Start the TUI first, then export the socket path:")
		fmt.Fprintln(os.Stderr, "      export GOYOKE_SOCKET=/run/user/1000/goyoke-poc-12345.sock")
		os.Exit(1)
	}
