...
// M-1: rules path is resolved from GOGENT_CONFIG_DIR or ~/.claude — not hardcoded.
...
// Uses GOGENT_CONFIG_DIR environment variable if set, otherwise falls back to ~/.claude.
func resolveRulesPath() string {
	configDir := os.Getenv("GOGENT_CONFIG_DIR")
	if configDir == "" {
...
func generateBioinformaticsPreSynthesis(result DetectionResult, rules InteractionRulesConfig, nReviewers int) string {
	...
	sb.WriteString("Generated by `gogent-team-prepare-synthesis`. " +
...
func appendInteractionSection(sb *strings.Builder, result DetectionResult, rules InteractionRulesConfig, nReviewers int) {
	...
	fmt.Fprintf(sb,
		"> Generated by gogent-team-prepare-synthesis using interaction-rules.json v%s.\n"+
