...
func TestExtractEinstein(t *testing.T) {
	tests := []struct {
		...
		{
			name:                   "truncation_root_causes",
			fixtureFile:            "truncated_einstein.json",
			wantExecutiveSummary:   "Testing truncation paths",
			wantRootCausesLen:      50, // Should truncate at maxRootCauses
			wantFrameworksLen:      1,
			wantFirstPrinciplesLen: 0,
			wantNovelApproachLen:   0,
			wantTradeoffsLen:       0,
			wantAssumptionsSurfLen: 0,
			wantOpenQuestionsLen:   0,
			wantHandoffNotes:       "Testing truncation",
			wantFallback:           false,
		},
		{
			name:                   "minimal_valid",
			fixtureFile:            "minimal_einstein.json",
			wantExecutiveSummary:   "Minimal valid Einstein output",
			wantRootCausesLen:      0,
			wantFrameworksLen:      0,
			wantFirstPrinciplesLen: 0,
			wantNovelApproachLen:   0,
			...
			wantAssumptionsSurfLen: 0,
			wantOpenQuestionsLen:   0,
			wantHandoffNotes:       "Minimal test",
			wantFallback:           false,
		},
		{
			name:                   "truncation_novel_approaches",
			fixtureFile:            "truncated_novel_approaches.json",
			wantExecutiveSummary:   "Testing novel approaches truncation",
			wantRootCausesLen:      0,
			wantFrameworksLen:      0,
			wantFirstPrinciplesLen: 0,
			wantNovelApproachLen:   20, // Should truncate at maxNovelApproaches
			wantTradeoffsLen:       0,
			wantAssumptionsSurfLen: 0,
			wantOpenQuestionsLen:   0,
			wantHandoffNotes:       "Testing novel approaches truncation",
			wantFallback:           false,
		},
	}
...
func TestExtractStaffArch(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			fixtureFile:           "no_recommendations_staff.json",
			wantExecutiveVerdict:  "**Verdict:** APPROVE",
			wantCriticalIssuesLen: 0,
			wantMajorIssuesLen:    0,
			wantMinorIssuesLen:    0,
			wantCommendationsLen:  1,
			wantFailureModesLen:   0,
			wantRecommendLen:      0,
			wantSignOffLen:        1,
			wantHandoffNotes:      "All good",
			wantFallback:          false,
		},
		{
			name:                  "no_signoff",
			fixtureFile:           "no_signoff_staff.json",
			wantExecutiveVerdict:  "**Verdict:** APPROVE",
			wantCriticalIssuesLen: 0,
			wantMajorIssuesLen:    0,
			wantMinorIssuesLen:    0,
			...
			wantRecommendLen:      0,
			wantSignOffLen:        0,
			wantHandoffNotes:      "No sign-off needed",
			wantFallback:          false,
		},
		{
			name:                  "no_executive_assessment",
			fixtureFile:           "no_executive_staff.json",
			wantExecutiveVerdict:  "",
			wantCriticalIssuesLen: 0,
			wantMajorIssuesLen:    0,
			wantMinorIssuesLen:    0,
			wantCommendationsLen:  1,
			...
			wantRecommendLen:      0,
			wantSignOffLen:        1,
			wantHandoffNotes:      "No executive assessment",
			wantFallback:          false,
		},
		{
			name:                  "truncation_issues",
			fixtureFile:           "truncated_staff_issues.json",
			wantExecutiveVerdict:  "**Verdict:** NEEDS_REVISION",
			wantCriticalIssuesLen: 100, // Should truncate at maxIssues
			wantMajorIssuesLen:    0,
			wantMinorIssuesLen:    0,
			wantCommendationsLen:  0,
			wantFailureModesLen:   0,
			wantRecommendLen:      0,
			wantSignOffLen:        0,
			wantHandoffNotes:      "Testing issue truncation",
			wantFallback:          false,
		},
	}
