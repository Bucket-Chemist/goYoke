// Package main implements the goyoke-aggregate CLI for weekly learning aggregation.
...
func main() {
	os.Exit(run(os.Args[1:], os.Getenv("GOYOKE_PROJECT_DIR")))
}
...
func run(args []string, projectDirEnv string) int {
	// Parse flags
	fs := flag.NewFlagSet("goyoke-aggregate", flag.ContinueOnError)
	...
	if err := fs.Parse(args); err != nil {
		fmt.Fprintf(os.Stderr, "[goyoke-aggregate] Failed to parse flags: %v\n", err)
		return 1
	}
	...
	if projectDir == "" {
		cwd, err := os.Getwd()
		if err != nil {
			fmt.Fprintf(os.Stderr, "[goyoke-aggregate] Failed to get working directory: %v\n", err)
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[goyoke-aggregate] Error checking files: %v\n", err)
		return 1
	}
	...
	if err := os.MkdirAll(archiveDir, 0755); err != nil {
		fmt.Fprintf(os.Stderr, "[goyoke-aggregate] Failed to create archive directory: %v\n", err)
		return 1
	}
	...
	if err := generateSummary(summaryPath, week, stats); err != nil {
		fmt.Fprintf(os.Stderr, "[goyoke-aggregate] Failed to generate summary: %v\n", err)
		return 1
	}
...
grep '"category":"architecture"' .goyoke/memory/archive/%s-decisions.jsonl
`+"```"+`

Review sharp edges:
`+"```bash"+`
# Example: Count error types
jq '.error_type' .goyoke/memory/archive/%s-sharp-edges.jsonl | sort | uniq -c
`+"```"+`
...
func printHelp(w *os.File) {
	...
	fmt.Fprintln(w, "  goyoke-aggregate                Check files and aggregate if >1MB")
	fmt.Fprintln(w, "  goyoke-aggregate --force        Force aggregation regardless of size")
	fmt.Fprintln(w, "  goyoke-aggregate --dry-run      Show what would be aggregated")
	fmt.Fprintln(w, "  goyoke-aggregate --help         Show this help")
	fmt.Fprintln(w, "")
	fmt.Fprintln(w, "Files checked:")
	fmt.Fprintln(w, "  - .goyoke/memory/pending-learnings.jsonl   (SharpEdges)")
	fmt.Fprintln(w, "  - .goyoke/memory/user-intents.jsonl        (UserIntents)")
	fmt.Fprintln(w, "  - .goyoke/memory/decisions.jsonl           (Decisions)")
	fmt.Fprintln(w, "  - .goyoke/memory/preferences.jsonl         (Preferences)")
	fmt.Fprintln(w, "  - .goyoke/memory/performance.jsonl         (Performance)")
	fmt.Fprintln(w, "  - .goyoke/memory/routing-violations.jsonl  (Violations)")
	fmt.Fprintln(w, "")
	fmt.Fprintln(w, "Archive location:")
	fmt.Fprintln(w, "  .goyoke/memory/archive/YYYY-Www-{type}.jsonl")
	fmt.Fprintln(w, "")
	fmt.Fprintln(w, "Examples:")
	fmt.Fprintln(w, "  goyoke-aggregate                    # Auto-aggregate if needed")
	fmt.Fprintln(w, "  goyoke-aggregate --dry-run          # Preview aggregation")
	fmt.Fprintln(w, "  goyoke-aggregate --force            # Force weekly rollup")
}
