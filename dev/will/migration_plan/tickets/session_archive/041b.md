...
- [ ] `GenerateWeeklySummary()` includes intent analysis
- [ ] Intent count by category calculated
- [ ] Percentages computed for category distribution
- [ ] Top 5 most frequent responses identified

### Recurring Preference Detection
- [ ] `DetectRecurringPreferences()` function implemented
- [ ] Preference = same category + similar response appearing 2+ times
- [ ] Similarity threshold: 80% string match or exact keyword overlap
- [ ] Returns list of (category, preference, frequency) tuples

### Preference Drift Detection
- [ ] `DetectPreferenceDrift()` function implemented
- [ ] Compares current week to previous week
- [ ] Detects new preferences (didn't exist last week)
- [ ] Detects dropped preferences (existed last week, not this week)
- [ ] Detects changed preferences (same category, different response)

### Markdown Output
- [ ] Weekly summary includes intent section (see format below)
- [ ] Category distribution table rendered
- [ ] Recurring preferences listed
- [ ] Drift alerts shown if any

### CLI Integration
- [ ] `gogent-archive weekly` shows intent summary
- [ ] `gogent-archive weekly --intents-only` shows only intent section
- [ ] `gogent-archive weekly --drift` highlights preference changes

### Testing
- [ ] Unit tests for DetectRecurringPreferences
- [ ] Unit tests for DetectPreferenceDrift
