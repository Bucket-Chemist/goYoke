...
description: "Track whether user intents were honored during the session. This closes the feedback loop: capture intent → act on it → record if we followed through. Enables learning effectiveness measurement, anti-pattern detection, and trust calibration."
status: pending
...
### Schema Extension
- [x] `Honored *bool` field added to UserIntent
- [x] `OutcomeNote string` field added to UserIntent
- [x] Both fields use `omitempty` for backward compatibility

### Outcome Analysis Engine
- [x] `AnalyzeIntentOutcomes()` function implemented
- [x] Correlates intents with subsequent actions
- [x] Determines if intent was honored based on action sequence
- [x] Generates outcome notes explaining reasoning

### Correlation Rules
- [x] Routing intents: Check if specified model/tier was used
- [x] Tooling intents: Check if specified tool was used/avoided
- [x] Workflow intents: Check if specified sequence occurred
- [x] Approval intents: Check if action proceeded
- [x] Rejection intents: Check if action was halted
- [x] Correction intents: Check if correction was applied

### SessionEnd Integration
- [x] Outcome analysis runs at session end
- [x] Updates user-intents.jsonl with Honored/OutcomeNote
- [x] Non-blocking (failure doesn't break session end)

### Query/CLI Extension
- [x] `UserIntentFilters.Honored` filter added
- [x] `gogent-archive user-intents --honored` shows honored intents
- [x] `gogent-archive user-intents --not-honored` shows unmet intents
- [x] Table output shows Honored column

### Weekly Integration
- [x] Honor rate included in weekly summary
- [x] Honor rate by category shown
- [x] Low honor rate categories flagged

### Testing
- [x] Unit tests for AnalyzeIntentOutcomes
- [x] Unit tests for each correlation rule
- [x] Integration test with mock session
- [x] `go test ./...` passes
