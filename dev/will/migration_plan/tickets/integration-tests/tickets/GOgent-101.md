...
description: **Task**: End-to-end testing for ML telemetry pipeline including routing decisions, collaborations, and export reconciliation.
status: pending
time_estimate: 2h
...
**Acceptance Criteria**:
- [ ] `TestMLTelemetry_RoutingDecisionCapture` verifies routing-decisions.jsonl created with valid JSON lines
- [ ] Routing decision file contains required fields: timestamp, routing_decision, tool_name
- [ ] `TestMLTelemetry_DecisionUpdates` verifies routing-decision-updates.jsonl is append-only
- [ ] Decision updates preserve monotonically increasing timestamps
- [ ] `TestMLTelemetry_ConcurrentWrites` spawns 5 parallel agents writing telemetry
- [ ] All 15 decisions (5 agents × 3 decisions) recorded without corruption
- [ ] No invalid JSON lines detected after concurrent writes
- [ ] `TestMLTelemetry_CollaborationTracking` verifies agent-collaborations.jsonl created
- [ ] Collaboration records include: agent_name, action, timestamp
- [ ] `TestMLTelemetry_ExportReconciliation` runs gogent-ml-export successfully
- [ ] Export generates: routing-decisions.csv, tool-sequences.json, agent-collaborations.csv, metadata.json
- [ ] CSV files have proper headers and data rows
- [ ] metadata.json contains: export_timestamp, decision_count, collaboration_count
- [ ] `TestMLTelemetry_RaceConditionDetection` passes with `go test -race`
- [ ] `TestMLTelemetry_SequenceIntegrity` verifies ml_telemetry fields propagate correctly
- [ ] All tests pass: `go test ./test/integration -v -run TestMLTelemetry`
- [ ] Race detector clean: `go test -race ./test/integration -run TestMLTelemetry`
- [ ] Code coverage ≥85% (ML telemetry is critical path)

**Test Deliverables**:
- [ ] Test file created: `test/integration/ml_telemetry_test.go`
- [ ] Test file size: ~500 lines
- [ ] Number of test functions: 7
- [ ] Coverage achieved: ≥85%
- [ ] Tests passing: ✅ (output: `go test ./test/integration -v -run TestMLTelemetry`)
- [ ] Race detector clean: ✅ (output: `go test -race ./test/integration -run TestMLTelemetry`)
- [ ] **ECOSYSTEM TEST PASS REQUIRED**: Run `make test-ecosystem` and verify ALL PASS
- [ ] Ecosystem test output saved to: `test/audit/GOgent-101/`
