...
- [ ] `TestRegression_LoadContext` validates gogent-load-context vs load-routing-context.sh on SessionStart events
- [ ] `TestRegression_LoadContext` reports ≥95% output parity with context injection validation
- [ ] `TestRegression_AgentEndstate` validates gogent-agent-endstate vs agent-endstate.sh on SubagentStop events
- [ ] `TestRegression_AgentEndstate` reports ≥95% output parity with endstate field validation
- [ ] `TestRegression_MLFieldPopulation` validates all ML fields populated at ≥95% rate across event types
- [ ] `TestRegression_MLFieldPopulation` includes per-event-type field population metrics
- [ ] `TestRegression_CollaborationFormat` validates JSONL schema consistency for all events
- [ ] `TestRegression_CollaborationFormat` verifies all required fields present (timestamp, session_id, event_type, etc)
- [ ] `TestRegression_ContextInjection` validates SessionStart context flows through dependent hooks
- [ ] All tests pass: `go test ./test/regression -v`
- [ ] Race detector clean: `go test -race ./test/regression`
- [ ] Test report shows detailed field population statistics and parity percentages per regression test

**Test Deliverables** (MANDATORY):
- [ ] Test file created: `test/regression/extended_regression_test.go`
- [ ] Test file size: ~650 lines
- [ ] Number of test functions: 5
- [ ] Coverage of new code: ≥80%
- [ ] Tests passing: ✅ (output: `go test ./test/regression -v`)
- [ ] Race detector clean: ✅ (output: `go test -race ./test/regression`)
- [ ] Extended tests pass with corpus: `export GOgent_CORPUS_PATH=... && go test ./test/regression -v`
- [ ] Test audit results saved to: `test/audit/GOgent-105/`
- [ ] Test INDEX.md updated with GOgent-105 entry
