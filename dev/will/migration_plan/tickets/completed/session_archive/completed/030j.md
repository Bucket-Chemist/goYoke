...
description: "Define ScoutRecommendation struct and implement LogScoutRecommendation() with dual-write pattern for tracking routing recommendations and compliance."
status: pending
...
## Acceptance Criteria
- [x] `ScoutRecommendation` struct defined per GAP document
- [x] `ScopeMetrics` struct captures file/line/token counts
- [x] `LogScoutRecommendation()` writes to global and project logs
- [x] `GetScoutLogPath()` returns XDG-compliant path
- [x] `LoadScoutRecommendations()` reads JSONL with missing file handling
- [x] Timestamp auto-populated in RFC3339 format
- [x] `Followed` field correctly captures compliance
- [x] Tests verify all scout operations
