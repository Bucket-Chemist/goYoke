...
## Acceptance Criteria
- [x] `ScoutAccuracyStats` struct defined
- [x] `CalculateScoutAccuracy()` returns accuracy metrics
- [x] `AnalyzeConfidenceCorrelation()` buckets by confidence level
- [x] `GetComplianceImpact()` compares followed vs ignored outcomes
- [x] `ClusterRecommendationsByTier()` shows tier recommendation distribution
- [x] Accuracy = (followed AND success) / total followed
- [x] Tests verify all accuracy calculations
