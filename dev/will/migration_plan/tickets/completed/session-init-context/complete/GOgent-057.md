...
**Acceptance Criteria**:
- [x] `GetToolCounterPath()` returns XDG-compliant path (NOT `/tmp`)
- [x] `InitializeToolCounter()` creates counter file with "0"
- [x] `GetToolCount()` returns 0 for missing file (not error)
- [x] `IncrementToolCount()` atomically increments using flock (upgraded from write-to-temp + rename for stronger atomicity)
- [x] Atomic locking prevents race conditions in concurrent hook execution
- [x] Tests verify initialization, increment, missing file handling
- [x] `go test ./pkg/config/...` passes (18/18 tests)
- [x] Path uses `~/.cache/gogent/` or `$XDG_CACHE_HOME/gogent/`

**Test Deliverables**:
- [x] Tests added to: `pkg/config/paths_test.go`
- [x] Number of new test functions: 5 (TestGetToolCounterPath, TestToolCounter_Initialize, TestToolCounter_GetCount_NotInitialized, TestToolCounter_Increment, TestToolCounter_ConcurrentIncrement)
- [x] Tests passing: ✅ (all 18 tests in pkg/config)
- [x] Race detector clean: ✅
- [x] Coverage: 73-100% for tool counter functions
- [x] Concurrent increment test validates 100 increments = exactly 100 (atomic flock prevents lost updates)
