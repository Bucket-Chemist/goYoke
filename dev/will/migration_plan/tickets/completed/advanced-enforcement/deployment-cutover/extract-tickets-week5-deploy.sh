...
# Ticket Extraction Script for Week 5 (deployment-cutover)
# Extracts individual tickets from 11-week5-deployment-cutover.md
# Special handling for GOgent-101b with letter suffix

SOURCE_FILE="11-week5-deployment-cutover.md"
OUTPUT_DIR="tickets"
...
for i in "${!ticket_ids[@]}"; do
    ...
    if [[ "$ticket_id" == "GOgent-101" ]]; then
        tags="[\"installation\", \"week-5\"]"
    elif [[ "$ticket_id" == "GOgent-101b" ]]; then
        tags="[\"wsl2\", \"week-5\"]"
    elif [[ "$ticket_id" == "GOgent-102" ]]; then
        tags="[\"testing\", \"week-5\"]"
    elif [[ "$ticket_id" =~ GOgent-10[3-5] ]]; then
        tags="[\"cutover\", \"week-5\"]"
    elif [[ "$ticket_id" =~ GOgent-10[6-8] ]]; then
