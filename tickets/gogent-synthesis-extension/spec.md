# Specification: Programmatic Cross-Domain Interaction Detection for gogent-team-prepare-synthesis
...
Add programmatic interaction detection to `gogent-team-prepare-synthesis` so that known cross-domain failure patterns are detected deterministically from wave 0 reviewer outputs, pre-computed, and included in `pre-synthesis.md` for the staff-bioinformatician to verify rather than discover.
...
gogent-team-prepare-synthesis (inter-wave script)
...
Wave 0 reviewers (parallel)
    ↓ produce stdout JSON files
gogent-team-prepare-synthesis (MODIFIED)
    ↓ reads stdout files
...
| `gogent-team-prepare-synthesis` | `cmd/gogent-team-prepare-synthesis/main.go` | Inter-wave Go binary |
...
```json
{
  ...
  "version": "1.0.0",
  "description": "Cross-domain interaction detection rules for gogent-team-prepare-synthesis",
  "rules": [
    {
...
> Generated by gogent-team-prepare-synthesis v{version} using interaction-rules.json v{rules_version}.
...
| `cmd/gogent-team-prepare-synthesis/interactions.go` | Interaction detection logic |
| `cmd/gogent-team-prepare-synthesis/interactions_test.go` | Unit tests for matching |
| `cmd/gogent-team-prepare-synthesis/testdata/interaction-rules.json` | Test fixture rules |
| `cmd/gogent-team-prepare-synthesis/testdata/mock-wave0/` | Mock reviewer outputs for integration tests |
| `cmd/gogent-team-prepare-synthesis/testdata/golden/` | Expected pre-synthesis.md outputs |

### 6.2 Modified Files

| File | Change |
|------|--------|
| `cmd/gogent-team-prepare-synthesis/main.go` | Load interaction-rules.json, run detection after finding extraction, append to pre-synthesis.md |
...
Programmatic detection in `gogent-team-prepare-synthesis`. Agent verifies pre-computed interactions + discovers novel ones.
