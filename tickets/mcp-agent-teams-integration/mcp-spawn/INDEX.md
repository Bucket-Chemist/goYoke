...
**Total Tickets**: 17
**Last Updated**: 2026-02-05 (added MCP-SPAWN-016, MCP-SPAWN-017 for orchestrators)
...
**CRITICAL**: 013 (Relationship Validation) must complete BEFORE 010/011/016/017 (Orchestrator Updates).
...
| MCP-SPAWN-016 | Impl-Manager Orchestrator Update | 2h | HIGH | [009, **013**] | pending |
| MCP-SPAWN-017 | Orchestrator Update (/plan) | 2h | HIGH | [009, **013**] | pending |
...
Phase 2 (CRITICAL PATH - validation before orchestrators):
  MCP-SPAWN-008 ─┬─► MCP-SPAWN-009 ─────────────────┬─► MCP-SPAWN-010 (Mozart)
                 │                                   ├─► MCP-SPAWN-011 (Review-Orch)
                 └─► MCP-SPAWN-013 ─────────────────┼─► MCP-SPAWN-016 (Impl-Manager)
                                                    └─► MCP-SPAWN-017 (Orchestrator)
  MCP-SPAWN-007 ────────────────────────────────────────► MCP-SPAWN-014

  Note: 009 and 013 can run in PARALLEL (both depend only on 008)
  010/011/016/017 require BOTH 009 AND 013 to complete before starting
  All four orchestrator updates (010/011/016/017) can run in PARALLEL

Phase 3:
  MCP-SPAWN-010 + 011 + 014 + 016 + 017 ─► MCP-SPAWN-012 ─► MCP-SPAWN-015
```
...
| Phase 2 | 14h | Orchestrator updates (4) + relationship validation |
| Phase 3 | 7h | Testing + docs + validation tests |
| **Total** | **40h** | ~5 days focused |

**With 50% buffer**: 22-30 days realistic timeline.
...
| 009 | 008 | 010, 011, 016, 017 |
| 010 | 009, **013** | 012 |
| 011 | 009, **013** | 012 |
| 012 | 010, 011, **014**, 016, 017 | 015 |
| 013 | 008 | 010, 011, 014, 016, 017 |
| 014 | 007, 013 | 012 |
| 015 | 012, 014 | - |
| 016 | 009, **013** | 012 |
| 017 | 009, **013** | 012 |
...
# IMPORTANT: In Phase 2, complete 013 BEFORE any orchestrator updates
cat MCP-SPAWN-013.md  # Do this first!
# Then orchestrator updates can run in parallel:
cat MCP-SPAWN-010.md  # Mozart (Braintrust)
cat MCP-SPAWN-011.md  # Review-Orchestrator (/review)
cat MCP-SPAWN-016.md  # Impl-Manager (implementation coordination)
cat MCP-SPAWN-017.md  # Orchestrator (/plan)
```
...
4. **Phase 3 Complete**:
   ...
   - 3 successful /plan workflow runs with orchestrator spawning
   - 3 successful /review workflow runs with review-orchestrator spawning
...
| 2026-02-05 | Added MCP-SPAWN-016 for impl-manager orchestrator update |
| 2026-02-05 | Added MCP-SPAWN-017 for general orchestrator update (/plan workflow) |
| 2026-02-05 | Added `spawned_by` field to staff-architect-critical-review in agents-index.json |
