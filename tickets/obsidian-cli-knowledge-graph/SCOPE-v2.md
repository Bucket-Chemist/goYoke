# GOgent-Fortress Embedded Knowledge Graph — Unified Scope (v2)

> **Status:** CONDITIONAL GO — 2 deployment blockers must be resolved before implementation begins
> **Source:** Synthesized from system_design.md + embedded_kg.md + Braintrust analysis + independent verification (2026-02-19)
> **Date:** 2026-02-19 (v2)
...
> Independent verification against upstream repositories and benchmarks produced the following material corrections. All changes are traceable to specific research findings.

| # | Section | Change | Severity |
|---|---------|--------|----------|
| Δ1 | §3 Blockers | **BLOCKER C-1 struck.** `coder/hnsw` is CC0-1.0 (public domain), not AGPL-3.0. The confusion arose from Coder's *platform* (`coder/coder`) being AGPL — the HNSW library has its own license. Blocker count: 3→2. | Critical |
| Δ2 | §5 Tech Stack | `chromem-go` license corrected: MPL-2.0, not MIT. Changed in v0.7.0 (Sep 2024). MPL-2.0 requires disclosure of modifications to MPL-licensed source files but permits use in proprietary binaries without modification. New compliance note added. | Major |
| Δ3 | §2.2 Not Building | `coder/hnsw` moved from "NOT included" to approved fallback. Replaces the "custom ~200-line HNSW using gonum" plan with a real, tested library under a public domain license. | Major |
| Δ4 | §5 Tech Stack | `fogfish/hnsw` (MIT) added as second HNSW option. Uses Go generics + pluggable `Surface[Vector]` interface + SIMD via `kshard/vector`. | Minor |
| Δ5 | §2.1, §8 | `sync.Map` replaced with `atomic.Pointer[map]` + copy-on-write. `sync.Map` is optimised for append-only caches and disjoint key access across goroutines — neither applies to ~10 core blocks with a read-heavy workload. `atomic.Pointer` gives zero-cost reads with trivial write path. | Minor |
| Δ6 | §10 Obsidian | goldmark extension stack consolidated: `powerman/goldmark-obsidian` (MIT) replaces 4 separate extensions. Covers wikilinks, hashtags, callouts, highlights, comments, block IDs, frontmatter, footnotes, LaTeX, Mermaid in one package. | Minor |
| Δ7 | §10 Obsidian | **Critical round-trip warning added.** goldmark is a Markdown→HTML converter whose AST discards formatting. Write strategy changed: goldmark for *parsing only*, direct string construction for vault *output*. | Major |
| Δ8 | §9 Consolidation | Entity resolution reframed: embedding cosine similarity is now the *primary* signal with OR-logic thresholds, not a secondary complement to Jaro-Winkler. Handles abbreviations and synonyms that string metrics score near zero on. | Minor |
| Δ9 | §7 Scoring | Noted Convex Combination (CC) as a proven upgrade path over RRF. Bruch et al. (ACM TOIS 2022) shows CC is more sample-efficient and robust to domain shift. Default remains RRF k=60 for v1. | Info |
| Δ10 | §16 Sharp Edges | Removed `agpl-license-coder-hnsw` edge (no longer applicable). Added `chromem-mpl2-compliance`, `goldmark-no-roundtrip`, `atomic-pointer-nil-init` edges. | Minor |
| Δ11 | §5 Tech Stack | `viterin/vek` (MIT) noted as SIMD acceleration layer for brute-force distance computation — extends chromem-go viability by 2-4x. | Info |
| Δ12 | Appendix A | Decisions table updated: "Cap at chromem-go, no coder/hnsw" replaced with corrected rationale. | Major |
...
> **Verdict:** Both tracks CONDITIONAL GO — proceed only after resolving two blockers below
...
| ~~**coder/hnsw (AGPL-3)**~~ | ~~Distribution poison pill~~ **[Δ1] STRUCK — coder/hnsw is CC0-1.0 (public domain).** Now approved as HNSW fallback. See §5 Tech Stack. | N/A |

---

## 3. Deployment Blockers (Must Resolve Before Implementation)

> These are structural prerequisites. No code should be written until both are resolved.
> **[Δ1] Reduced from 3 to 2 blockers.** C-1 (AGPL license contradiction) struck — coder/hnsw is CC0-1.0.

### ~~BLOCKER C-1: AGPL License Contradiction~~ — RESOLVED (Not a Blocker)

> **[Δ1] This blocker was based on incorrect license identification.** `github.com/coder/hnsw` is licensed CC0-1.0 (public domain dedication), not AGPL-3.0. The confusion arose from Coder's *platform product* (`coder/coder`) being AGPL-3.0 — the HNSW library is a separate repository with its own license file. Verified against `github.com/coder/hnsw/blob/main/LICENSE`.
>
> `coder/hnsw` is now approved as a fallback dependency. No action required.
...
- Approved dependencies (with license annotations — corrected per Δ1, Δ2)
...
> **[Δ1, Δ2, Δ4, Δ5, Δ6, Δ11] Corrected license column. Added fallback HNSW options. Replaced sync.Map. Consolidated goldmark extensions.**
...
| **Markdown parser** | `github.com/powerman/goldmark-obsidian` [Δ6] | MIT | Unified Obsidian extension: wikilinks, hashtags, callouts, highlights, comments, block IDs, frontmatter, footnotes. **Parsing only — not for round-trip rendering.** |
...
| **SIMD acceleration** | `github.com/viterin/vek` [Δ11] | MIT | SIMD dot products for brute-force distance computation. Can extend chromem-go viability by 2-4x. |
...
**Previously listed (now consolidated into powerman/goldmark-obsidian):**
- ~~`go.abhg.dev/goldmark/wikilink`~~ (BSD-3) — covered by powerman
- ~~`go.abhg.dev/goldmark/frontmatter`~~ (MIT) — covered by powerman
- ~~`go.abhg.dev/goldmark/hashtag`~~ (MIT) — covered by powerman
- ~~`github.com/VojtaStruhar/goldmark-obsidian-callout`~~ (MIT) — covered by powerman
...
Use `powerman/goldmark-obsidian` — a single package covering wikilinks (including aliases and fragments), `![[embeds]]`, `#hashtags` (ObsidianVariant), `> [!note]` callouts (collapsible + nested), `==highlights==`, `%%comments%%`, block IDs, YAML frontmatter, footnotes, and LaTeX. Internally uses `abhinav/goldmark-wikilink` and `abhinav/goldmark-hashtag`.
...
1. ~~Resolve C-1: Remove `coder/hnsw` from approved dependencies~~ **[Δ1] No longer needed — CC0-1.0**
...
5. Update license annotations in agent definition per §5 corrections [Δ1, Δ2]
6. Resolve M-1: Create agents-index.json entry + add to can_spawn lists
7. Validate routing: `gogent-validate routes to go-db-architect for "graph schema" trigger`
...
SQLite schema, atomic.Pointer[map] cache, IR-based scoring, basic CRUD + search.
...
Write-only vault export (string construction), batch import (goldmark parsing), consolidation pipeline.

**Decision points before starting:**
- Validate `powerman/goldmark-obsidian` parsing fidelity (test vault with 50 files) [Δ6]
...
- [ ] Entity resolution uses embedding-first OR-logic (§9) [Δ8]
...
| Does `powerman/goldmark-obsidian` parse all vault file variants correctly? [Δ6] | MEDIUM | Before Phase 3 | Parse 50 representative vault files; diff against expected structure |
...
| chromem-go degrades beyond 50K vectors before anticipated | Medium | Medium | Benchmark at 100K vectors (768d) before committing; if fails, swap to `coder/hnsw` (CC0) or `fogfish/hnsw` (MIT) behind interface [Δ1, Δ3] |
...
| goldmark round-trip produces lossy output for Obsidian syntax [Δ7] | ~~Medium~~ **Eliminated** | ~~Medium~~ | **Mitigated by design:** vault export uses string construction, not goldmark rendering. goldmark used only for parsing inbound human edits. |
| chromem-go single-maintainer risk — project goes unmaintained | Medium | Medium | Interface abstraction allows swap to fogfish/hnsw or coder/hnsw. Monitor repo activity quarterly. [Δ3, Δ4] |
| chromem-go per-document persistence creates 100K files on disk | Medium | Low | Use `ExportToFile` for single-file persistence instead of per-document mode |
...
| MPL-2.0 compliance missed if chromem-go source is modified [Δ2] | Low | Medium | Track in LICENSE-THIRD-PARTY; code review gate for any chromem-go source modifications |

---

## 16. Sharp Edges (v1 Active, v2 Deferred)

> **[Δ10] After Einstein's simplifications + license corrections:** ~15-17 active edges (from 25 original). bbolt edges (~5) and bidirectional vault sync edges (~3) deferred to v2 documentation. AGPL edge struck (Δ1). Three new edges added.
...
- `chromem-mpl2-compliance` [Δ2, Δ10]: chromem-go is MPL-2.0. Modifications to its source files must be disclosed. Unmodified dependency use requires no disclosure. Track in LICENSE-THIRD-PARTY.
...
- `goldmark-no-roundtrip` [Δ7, Δ10]: goldmark is a Markdown→HTML converter. Its AST discards formatting. **Never use goldmark to write vault files.** Use string construction for output, goldmark for parsing only.
- ~~`goldmark-obsidian-highlights`~~: **Covered by `powerman/goldmark-obsidian`** [Δ6]
- ~~`goldmark-round-trip-fidelity`~~: **Mitigated by write strategy change** [Δ7]
...
**Cache:** [Δ5, Δ10]
- `atomic-pointer-nil-init`: `atomic.Pointer[map]` starts as nil. Always check `cache.Load() != nil` before accessing, or initialise with empty map at construction time.
...
│   ├── parse.go              -- powerman/goldmark-obsidian parsing  [Δ6]
...
│   ├── resolve.go            -- Entity resolution (embedding-first OR-logic)  [Δ8]
...
| ~~Cap at chromem-go, no coder/hnsw~~ | ~~AGPL-3 license violation~~ **[Δ1] Corrected.** coder/hnsw is CC0-1.0. Now approved as fallback. chromem-go remains primary (exhaustive search sufficient for ≤100K). Fallback to coder/hnsw or fogfish/hnsw behind VectorIndex interface at >100K. | chromem-go benchmark succeeds at 100K AND no latency improvement needed |
| goldmark for parsing only, string construction for writing [Δ7] | goldmark AST discards Obsidian formatting; round-trip is lossy | goldmark adds a Markdown renderer that preserves Obsidian syntax perfectly |
| powerman/goldmark-obsidian as single parsing extension [Δ6] | Covers wikilinks, hashtags, callouts, highlights, comments, block IDs in one package | Extension fails to parse a required Obsidian syntax element |
