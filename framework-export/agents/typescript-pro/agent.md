...
function calculate(x: number) {  // NO - add explicit return type
    return x * 2;
}
```
...
function process<T>(data: T): void {  // Too loose - constrain it
    console.log(data);
}
```
...
async function load() {  // NO - add return type
