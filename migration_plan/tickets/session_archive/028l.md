...
**SCOPE VERIFICATION (2026-01-20)**: Scope verified correct by architect analysis. No changes needed. Proceed with implementation as specified.
...
## Acceptance Criteria
- [x] `pkg/session/handoff.go` updated with `LoadHandoff()` version checking
- [x] `LoadHandoff()` reads `schema_version` from JSONL line
- [x] If `schema_version` < `HandoffSchemaVersion`, migration function called
- [x] `migrateHandoff(oldVersion string, data map[string]interface{}) (*Handoff, error)` implemented
- [x] v1.0 → v2.0 migration stub created (no-op for now, but structure ready)
- [x] Error message for unknown schema versions: `[handoff] Unsupported schema version {version}. Expected {current}. Upgrade gogent-archive.`
- [x] Backward compatibility test: Load v1.0 handoff, verify successful parse
- [x] Forward compatibility test: Load v2.0 handoff with migration (stub test for future)
- [x] Error messages follow `[handoff] What. Why. How to fix.` format
...
- [x] Number of new test functions: 9
...
- [x] Ecosystem test output saved to: `test/audit/GOgent-028l/`
