...
### Schema Updates
- [ ] SharpEdge struct extended in `pkg/session/handoff.go`:
...
- [ ] Existing fields preserved exactly (backward compatible)
- [ ] JSON tags match existing JSONL format

### Validation Reconciliation
- [ ] `ValidateSharpEdge()` updated to use correct field names:
  - `consecutive_failures` (not `failure_count`)
  - `timestamp` (not `last_occurrence`)
- [ ] New optional fields validated when present

### Query API
- [ ] `pkg/session/query.go` file created
- [ ] `Query` struct with `ProjectDir string` field
- [ ] `NewQuery(projectDir string) *Query` constructor
- [ ] `SharpEdgeFilters` struct:
...
- [ ] `QuerySharpEdges(filters SharpEdgeFilters) ([]SharpEdge, error)` implemented
- [ ] Missing file returns empty slice (not error)
- [ ] Malformed lines skipped gracefully

### Formatting Functions
- [ ] `FormatSharpEdge(edge SharpEdge) string` returns single markdown line
- [ ] `FormatSharpEdges(edges []SharpEdge) string` returns full markdown section
- [ ] Format includes ErrorMessage when present (truncated to 100 chars)
- [ ] Format includes Severity badge when present
- [ ] Backward compatible with old JSONL (missing fields = empty)

### Test Coverage
- [ ] `pkg/session/query_test.go` created with ≥90% coverage
- [ ] Tests for: QuerySharpEdges with all filter combinations
- [ ] Tests for: Backward compatibility with old JSONL format
- [ ] Tests for: Missing file handling
- [ ] Tests for: Malformed line skipping
...
- [ ] Test file created: `pkg/session/query_test.go`
