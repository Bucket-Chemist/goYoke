...
## Acceptance Criteria
- [ ] Script created: `scripts/session-archive-wrapper.sh`
- [ ] Script reads STDIN (preserves for retry)
- [ ] Attempts `gogent-archive` execution
- [ ] If exit code 0, outputs Go result and exits
- [ ] If exit code non-zero, logs failure to stderr
- [ ] Falls back to `~/.claude/hooks/session-archive.sh`
- [ ] Passes original STDIN to bash hook
- [ ] Logs which path succeeded to `~/.gogent/hook-failures.log`
- [ ] Install target copies wrapper to `~/.claude/hooks/`
- [ ] Test script simulates Go failure → verifies bash fallback
...
- [ ] Wrapper script created: `scripts/session-archive-wrapper.sh`
- [ ] Makefile target `install-wrapper` added
- [ ] Test script created: `test/integration/fallback_test.sh`
- [ ] Test passes: `bash test/integration/fallback_test.sh`
- [ ] Manual test: Rename gogent-archive binary, verify bash fallback
- [ ] **ECOSYSTEM TEST PASS REQUIRED**: Run `make test-ecosystem` and verify ALL PASS
- [ ] Ecosystem test output saved to: `test/audit/GOgent-028i/`
