...
## Acceptance Criteria
- [ ] `AgentInvocation` struct defined with all fields per GAP document
- [ ] `LogInvocation()` writes to global XDG cache
- [ ] `LogInvocation()` writes to project memory (graceful degradation)
- [ ] `GetInvocationsLogPath()` returns XDG-compliant path
- [ ] `GetProjectInvocationsLogPath(projectDir)` returns project-scoped path
- [ ] Missing directory auto-created
- [ ] Timestamp auto-populated in RFC3339 format
- [ ] Tests verify dual-write pattern
- [ ] Tests verify graceful degradation on project write failure
