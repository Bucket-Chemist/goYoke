...
## Acceptance Criteria
- [ ] Makefile target `build-archive` added: `go build -o bin/gogent-archive ./cmd/gogent-archive`
- [ ] Makefile target `install-archive` copies binary to `~/.local/bin/gogent-archive`
- [ ] Hook configuration documented in `docs/deployment/hook-registration.md`
- [ ] Example hook config shows: `command = "gogent-archive"` or absolute path
- [ ] Integration test created: `test/integration/session_end_hook_test.go`
- [ ] Test simulates SessionEnd JSON on STDIN → gogent-archive → JSONL + MD outputs
- [ ] Test verifies handoff files created at expected paths
- [ ] Test verifies artifacts archived to session-archive/
- [ ] Test verifies JSON confirmation output matches schema
- [ ] Rollback procedure documented: restore bash hook, verify works
- [ ] All tests pass: `go test ./test/integration -run TestSessionEndHook`
- [ ] Binary builds successfully: `make build-archive`
...
- [ ] Test file created: `test/integration/session_end_hook_test.go`
- [ ] Number of test functions: 2
- [ ] Tests passing: ✅ (output: `go test ./test/integration -run TestSessionEndHook`)
- [ ] Makefile targets work: `make build-archive && make install-archive`
- [ ] Documentation file created: `docs/deployment/hook-registration.md`
