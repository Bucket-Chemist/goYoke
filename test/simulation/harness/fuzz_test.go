...
func TestFuzzRunner_SetupRandomArtifacts(t *testing.T) {
	...
	os.MkdirAll(filepath.Join(testDir, ".claude", "memory"), 0755)
	...
	for seed := int64(0); seed < 100; seed++ {
		...
		violationsPath := filepath.Join(iterDir, ".claude", "memory", "routing-violations.jsonl")
		edgesPath := filepath.Join(iterDir, ".claude", "memory", "pending-learnings.jsonl")
