...
- [ ] Create project: `go mod init github.com/yourname/GoGent`
- [ ] Add dependencies:
  ```bash
...
## Phase 3: GoGent CLI v0.1 (Month 2)

### Core Commands

- [ ] `GoGent list` - Show active agents
- [ ] `GoGent hook create <id>` - Create hook directory
- [ ] `GoGent hook read <id>` - Read current_work.json
- [ ] `GoGent status` - Overall system status

### Session Management

- [ ] `GoGent session start <agent>` - Spawn tmux session
- [ ] `GoGent session stop <id>` - Kill session
- [ ] `GoGent session attach <id>` - Attach to session

### Integration

- [ ] Update orchestrator to call GoGent CLI
...
- [ ] `GoGent spawn --background` implementation
- [ ] `GoGent wait --any/--all` implementation
...
# Test GO TUI
go run cmd/GoGent-tui/main.go
```

### After Phase 3 (GoGent CLI)

```bash
# Test GoGent commands
GoGent list
GoGent hook create test-agent
GoGent hook read test-agent
GoGent status
```
...
| P3    | `rm $(which GoGent)`             | GoGent CLI                 |
| P4-6  | Disable GoGent, re-enable Task() | Advanced coordination      |
