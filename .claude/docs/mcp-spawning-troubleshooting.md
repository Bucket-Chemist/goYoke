...
### 4b. "can only be spawned by [X], not 'router'" (CRITICAL)

**Cause**: Task-spawned agent (like Mozart) is trying to spawn children without self-identifying.

**Error Message**:
```json
{
  "success": false,
  "error": "Spawn validation failed: 'einstein' can only be spawned by [mozart], not 'router'",
  "validationErrors": [
    {
      "code": "E_SPAWNED_BY_VIOLATION",
      "message": "'einstein' can only be spawned by [mozart], not 'router'"
    }
  ]
}
```
...
Mozart is spawned via Task() (Claude Code's built-in subagent system), NOT via spawn_agent.
When Mozart calls spawn_agent, the validation can't determine Mozart's identity from the store
(because Task-spawned agents aren't registered in the MCP store), so it defaults to "router".
...
6. Einstein's spawned_by: ["mozart"] doesn't include "router"
7. VALIDATION FAILS
```

**Solution - Add `caller_type` Parameter**:
...
**Agents That Need `caller_type`**:

| Agent | Spawned By | When Spawning | Needs caller_type? |
|-------|------------|---------------|-------------------|
| Mozart | Task() from /braintrust | Einstein, Staff-Architect, Beethoven | **YES** |
| Review-Orchestrator | Task() from /review | backend-reviewer, frontend-reviewer, etc. | **YES** |
| Orchestrator | Task() | Various | **YES** |
| Einstein | spawn_agent from Mozart | None (terminal) | No |
| Beethoven | spawn_agent from Mozart | None (terminal) | No |

**How `caller_type` Validation Works**:

When `caller_type` is provided, the system performs **bidirectional validation**:
1. Does child's `spawned_by` include the claimed `caller_type`? ✓
2. Does claimed `caller_type`'s `can_spawn` include the child? ✓

Both must pass. This prevents arbitrary agents from claiming to be Mozart.
