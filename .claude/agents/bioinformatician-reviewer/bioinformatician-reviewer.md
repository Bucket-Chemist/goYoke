...
|---|-------|---------------|----------------|-----------------|-----|
| 1 | Container images pinned by SHA256 digest | NF: `container 'image@sha256:'`; WDL: `docker: "image@sha256:"`; SM: `container: "image@sha256:"` (Snakemake 8+) or `singularity: "image.sif"` with recorded digest; check `--singularity-prefix` shared cache for tag-only pulls | Tag `latest` or `v1.0` resolves to different image over time | Pipeline produces different results next month with identical code and data; irreproducible publication | `[CODE]` |
...
> **Note on #8:** In Nextflow, `errorStrategy 'ignore'` causes the failed process to emit an empty output channel. Downstream processes that call `.collect()` include the empty entry — a `merge_results` step over 100 samples silently produces results from 99. The safer pattern: `errorStrategy { task.exitStatus in [143,137,104,134,139] ? 'retry' : 'finish' }` — retry transient OOM/signal kills, fail on logical errors. In Snakemake, `shell:` blocks execute via `/bin/sh` where piped commands mask intermediate failures — only the last command's exit code is returned. Require `set -euo pipefail` at the start of every shell block. Additionally, `--keep-going` allows independent rules to continue after a failure, producing partial result sets that appear complete but silently omit failed samples — look for `--keep-going` or `-k` in invocation scripts. `run:` blocks (Python) propagate exceptions correctly but swallowed `try/except: pass` has the same masking effect. In WDL, the `command { ... }` block returns the exit code of the LAST command only — intermediate failures are silently lost (identical to bash without `set -e`). There is no WDL-native equivalent of `set -euo pipefail`; it must be added explicitly as the first line inside the command block. Preemptible task retries on spot interruption share the same retry mechanism as retries on task failure — Cromwell logs cannot distinguish "killed by cloud provider" from "failed due to bug," making debugging preemptible failures in WDL workflows particularly difficult.

> **Note on #9:** Nextflow `-resume` hashes inputs + script + container → cache key. But `publishDir mode: 'move'` physically moves output from the work directory. On resume, the cache hash matches but the file is gone → silent re-execution that may pull a different container image if the tag has moved. Use `mode: 'copy'` or `mode: 'link'` with `-resume`. Snakemake's default `--rerun-triggers mtime` means a file whose content changed but whose timestamp didn't (e.g., overwritten by `cp --preserve=timestamps`) is not reprocessed. `--rerun-incomplete` marks rules with incomplete output for re-execution but doesn't verify content correctness. `checkpoint` rules (dynamic output that determines downstream DAG structure) have their own re-evaluation semantics — when a checkpoint output changes, Snakemake re-evaluates the entire downstream DAG, which can cause unexpected cascading re-runs. `--touch` can mark outputs as complete without running rules — a manual "fake completion" mechanism that bypasses all validation. In WDL, call caching behavior diverges across backends: Cromwell caches by task hash (`call-caching.enabled = true` in config), miniwdl caches by input content hash (`--copy-input-files` affects cache behavior), and Terra's call cache is workspace-scoped — different users in the same workspace share the cache, creating potential cross-contamination where one user's cached output is served to another user with different inputs if the hash collides.
...
> **Note on #19:** Batch effect correction passes the Substitution Test because it applies regardless of analysis domain. Whether you're doing RNA-seq, proteomics, or metabolomics, uncorrected batch effects produce the same class of artifact: systematic technical variation masquerading as biological signal. The code-verifiable component: grep for `ComBat`, `removeBatchEffect`, `sva`, or `batch` as a covariate in linear model formulas (`~ treatment + batch`). The design-verifiable component: is batch confounded with treatment group? This requires study metadata (sample sheet with batch and condition columns). If the sample sheet is in the repository, check whether any batch contains only one condition — if so, batch and condition effects are inseparable. If the sample sheet is not in the repository, output "Recommend manual review: verify batch-treatment independence from experimental design.".
