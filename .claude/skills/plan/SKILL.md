...
```
[plan] Starting comprehensive planning workflow.
[plan] Model tier: opus (32K thinking budget per phase)
```
...
STRATEGY: <contents of .claude/tmp/strategy.md>
...
STRATEGY FILE: .claude/tmp/strategy.md (for context)
...
```bash
# Read review verdict
verdict=$(jq -r '.verdict' .claude/tmp/review-metadata.json)
critical_count=$(jq -r '.issue_counts.critical' .claude/tmp/review-metadata.json)
```
...
```
[plan] Critical issues found in review.
[plan] Issues:
- C-1: <issue summary>
- C-2: <issue summary>
...
```
[plan] GAP document ready: .claude/tmp/einstein-gap-<timestamp>.md

Run /einstein to resolve, then /plan --resume to continue.
```
...
6. None → error "No checkpoint found. Run /plan to start fresh."
```
...
The tickets generated by /plan are fully compatible with /ticket skill:
...
[plan] Starting comprehensive planning workflow.
[plan] Model tier: opus (32K thinking budget per phase)
[plan] Goal: "Add JWT authentication to the API"
...
Review saved to: .claude/tmp/review-critique.md
